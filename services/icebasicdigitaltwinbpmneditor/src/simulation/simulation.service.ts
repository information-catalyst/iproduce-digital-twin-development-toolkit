...
export interface IFinishedTask {
  pid?: string;
  pname?: string;
  task: string;
  startTime: string;
...
export interface ISimulationService {
  ...
  getFinishedTasks(): IFinishedTask[];
...
class SimulationService implements ISimulationService {
  ...
  private _finishedTasks: IFinishedTask[];
  ...
  private wsPdeLogs: WebSocket;
  private wsBpmnData: WebSocket;
  private sendFn;
  private sendLog;
  private sendStartEventFn;
  private sendEndEventFn;
  private singleEndMessage = true;
  ...
  private getLogEntry(thread): IFinishedTask {
    let temp = <IFinishedTask>{};
    if (thread.element.name !== undefined) {
      temp = {
        pid: this._managerService.getModel()._id,
        pname: this._managerService.getModel().processName,
        task: thread.element.name,
        startTime: new Date().toLocaleDateString("en-GB") + " " + new Date().toLocaleTimeString("en-GB")
      };
    }
    return temp;
  }

  private addTask(thread): void {
    ...
    // if (thread.element.name !== undefined) {
      ...
      this._finishedTasks.push(this.getLogEntry(thread));
    // }
  ...
  private connect() {
    ...
    /*
        ...
        // ipaddr = "icemain.hopto.org"; // TO-DO change this after testing
        console.log ("ipaddr" + ipaddr );
    */
    let ipaddr = window.location.hostname;
    // strip any www prefix, TODO regex would be good here
    ipaddr = ipaddr.replace("www.", "");
    console.log("ipaddr" + ipaddr);

    // ipaddr = "icemain.hopto.org";
//    const wsurl = "ws://" + ipaddr + ":7061/v2/broker/?topics=commands";
//    console.log("wsurl" + wsurl);

    // we can consume frommultiple topices with one conenction!
    // sending to  topic is not defined yt the connetion url , so we can avoid multiple connections
    const wsurl = "ws://" + ipaddr + ":7061/v2/broker/?topics=commands,bpmndata,pdelogs";
    console.log("wsurl" + wsurl);
/*
    const wsBpmnUrl = "ws://" + ipaddr + ":7061/v2/broker/?topics=bpmndata";
    const wsPdeLogsUrl = "ws://" + ipaddr + ":7061/v2/broker/?topics=pdelogs";
*/
    // Direct to ICEMAIN
    ...
    this.ws.onopen = function (evt) {
      console.log("WebSocket connection made to commands topic");
    };
/*    

    this.wsBpmnData = new WebSocket(wsBpmnUrl);
    this.wsBpmnData.onopen = function (evt) {
      console.log("WebSocket connection made to bpmndata topic");
    };

    this.wsPdeLogs = new WebSocket(wsPdeLogsUrl);
    this.wsPdeLogs.onopen = function (evt) {
      console.log("WebSocket connection made to pdelogs topic");
    };
*/

    /**
     * This will send simulation log entry to pdelogs kafka topic
     */
    this.sendLog = function (data) {
      const timenow = Date.now();
      const payload = JSON.stringify({ "source": "PDE", "logmessage": this.getLogEntry(data), "action": "bpmnLog", "timestamp": timenow, "processid": this.randomNumber });
      const msg = { "topic": "pdelogs", "message": payload };
//      this.wsPdeLogs.send(JSON.stringify(msg));
    ...
    let xmlHttp;
      ...
      xmlHttp.send("");
      return Date.parse (xmlHttp.getResponseHeader("Date"));
  };
    ...
    this.sendFn = function (data) {
      ...
      const timenow = Date.now();
      const payload = JSON.stringify({ "source": "PDE", "task": data, "action": "start", "timestamp": timenow, "processid": this.randomNumber });
    ...
    this.sendStartEventFn = function (data) {
      //      const msg = {"topic": "commands", "message": data};
      const timenow = Date.now();

      this._managerService.getModeler().getXML().then((xml) => {
        const xmlPayload = JSON.stringify({ "source": "PDE", "rawxml": xml, "timestamp": timenow, "processname": this._managerService._currentModel["processName"], "processid": this.randomNumber });
        const xmlMsg = { "topic": "bpmndata", "message": xmlPayload };
        this.ws.send(JSON.stringify(xmlMsg));
      });


      const payload = JSON.stringify({ "source": "PDE", "event": data, "action": "start", "timestamp": timenow, "processname": this._managerService._currentModel["processName"], "processid": this.randomNumber });
      const msg = { "topic": "commands", "message": payload };
      console.log("Sending " + JSON.stringify(msg));

      // console.log ("Sending " + this._managerService._currentModel['processName']);
      // console.log("xxxx"+ProcessMessageType.Information);

      this.ws.send(JSON.stringify(msg));
      this.singleEndMessage = true; // allow an end event message to be sent
      //      msg.message = "";
    };


    this.sendEndEventFn = function (data) {
      //      const msg = {"topic": "commands", "message": data};
      if (this.singleEndMessage) {

        this.singleEndMessage = false;
        const timenow = Date.now();
        const payload = JSON.stringify({ "source": "PDE", "event": data, "action": "start", "timestamp": timenow, "processname": this._managerService._currentModel["processName"], "processid": this.randomNumber });
        const msg = { "topic": "commands", "message": payload };
        console.log("Sending " + JSON.stringify(msg));
        ...
        // alread sent an end message
      }
      //      msg.message = "";
    };

    this.ws.onerror = function (evt) {
      console.log("An error occured while connecting to commands topic... " + evt);
    };
...
/*
    this.wsPdeLogs.onerror = function (evt) {
      console.log("An error occured while connecting to pdelogs topic... " + evt);
    };


    this.wsBpmnData.onerror = function (evt) {
      console.log("An error occured while connecting to bpmndata topic... " + evt);
    };
*/
  }
  ...
  private processThread(thread: ISimulationThread): void {
    ...
    if (nextElement || thread.element.id !== null) {
      const _self = this;
      let sentItem = thread.element.id;

      if (sentItem.startsWith("StartEvent")) {
        this.randomNumber = this.getRandomNumber();              /// give each execution process a unique number
        sentItem = sentItem + "#" + this.randomNumber;
        this.sendStartEventFn(sentItem);
        // we will handle start and stop here
        _self.resumeTimer();
        _self.removeMarker(thread);
        thread.element = nextElement;
        _self.markAndCenter(thread);
      } else if (sentItem.startsWith("EndEvent")) {
        sentItem = sentItem + "#" + this.randomNumber;
        this.sendEndEventFn(sentItem);
        ...
        this.ws.onmessage = function (resp) {
          ...
          const payload = JSON.parse(JSON.parse(resp.data).message);


          const hasTask = payload.hasOwnProperty("task");
          const hasAction = payload.hasOwnProperty("action");
          // ignore irrelevant command messages
          if (hasTask && hasAction) {
  ...
  public clearFinishedTasks(): void {
    this._finishedTasks.splice(0, this._finishedTasks.length - 1);
